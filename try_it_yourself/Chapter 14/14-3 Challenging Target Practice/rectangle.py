...
class Rectangle:
    ...
    def _check_top_edge(self):
        return self.rect.top < self.settings.rectangle_width

    def _check_bottom_edge(self):
        return (
            self.rect.bottom > self.screen_rect.height - self.settings.rectangle_width
        )
    ...
    def update(self):
        ...
        self.rect.y = self.y
        if self._check_top_edge():
            self.settings.rectangle_direction *= -1

            # Speed the game up each time the rectangle hits the top of the screen.
            self.settings.increase_speed()

        elif self._check_bottom_edge():
            self.settings.rectangle_direction *= -1
